...
--     SET geometrie = ST_MakeValid(ST_RemoveRepeatedPoints(ST_ReducePrecision(ST_Buffer(geometrie, 0.02), 0.01), 0.01))      -- Test mit awjf_work.wap_bst_bkt und awjf.wap_bst_es > geht
--     SET geometrie = ST_MakeValid(ST_RemoveRepeatedPoints(ST_ReducePrecision(geometrie, 0.01), 0.01))      -- Test mit awjf_work.wap_bst_bkt und awjf.wap_bst_es > geht nicht
--     SET geometrie = ST_MakeValid(ST_RemoveRepeatedPoints(ST_ReducePrecision(geometrie, 0.001), 0.001))      -- Test mit awjf_work.wap_bst_bkt und awjf.wap_bst_es > geht nicht

     SET geometrie = ST_RemoveRepeatedPoints(ST_ReducePrecision(ST_Buffer(geometrie, 0.001), 0.001), 0.001)                   -- geht bei awjf_work.wap_bst_bkt und awjf.wap_bst inkl. check_ili_export

--     SET geometrie = ST_RemoveRepeatedPoints(ST_ReducePrecision(geometrie, 0.001), 0.001)                   -- awjf_work.wap_bst_es: ERROR: Geometry type (MultiPolygon) does not match column type (Polygon)
--     SET geometrie = ST_RemoveRepeatedPoints(ST_ReducePrecision(geometrie, 0.001), 0.001)                   -- awjf_work.wap_bst_es: ERROR: Geometry type (MultiPolygon) does not match column type (Polygon)
;
